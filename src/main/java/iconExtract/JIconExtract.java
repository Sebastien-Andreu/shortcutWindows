...
public class JIconExtract {
    ...
    public static BufferedImage getIconForFile(int width, int height, String fileName) {
        ...
        try {
            ...
            HDC hdc = User32.INSTANCE.GetDC((HWND)null);
            ...
            bitmapinfo.read();
            Memory lpPixels = new Memory((long)bitmapinfo.bmiHeader.biSizeImage);
    ...
    public static HBITMAP getHBITMAPForFile(int width, int height, String fileName) {
        HRESULT h1 = Ole32.INSTANCE.CoInitialize((LPVOID)null);
        if (COMUtils.SUCCEEDED(h1)) {
            PointerByReference factory = new PointerByReference();
            HRESULT h2 = Shell32Extra.INSTANCE.SHCreateItemFromParsingName(new WString(fileName), (Pointer)null, new REFIID(new IID("BCC18B79-BA16-442F-80C4-8A59C30C463B")), factory);
